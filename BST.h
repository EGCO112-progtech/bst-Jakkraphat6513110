...
void printTree(int value, int space)
{
   while (space)
   {
      printf("    ");
...
void reverseinOrder( TreeNodePtr treePtr, int counter )
{

   // if tree is not empty, then traverse
   if ( treePtr != NULL ) {        
      reverseinOrder( treePtr->rightPtr, ++counter ); //Recursion to the right
      printTree(treePtr->data, counter);
      reverseinOrder( treePtr->leftPtr, counter ); //Recursion to the left
